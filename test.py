...
def draw_text(surface, text, x, y, color=WHITE, bg_color=None):
    ...
    return text_rect.height # Return text height for layout
...
def draw_axis_status(surface, axis_idx, axis_value, x, y, available_width):
    ...
    text_height = draw_text(surface, f"Axis {axis_idx}: {display_value}", x, base_y)

    # Visual Bar Configuration
    bar_label_gap = 10 # Gap between label and bar
    bar_width = max(50, available_width - (x + PADDING) - len(display_value)*FONT_SIZE*0.6 ) # Estimate text width, ensure min bar width
    bar_height = FONT_SIZE * 0.8
    bar_x = x + available_width - bar_width - PADDING # Align bar to the right within the column
    bar_y = base_y + (text_height - bar_height) / 2 # Vertically align with text
    ...
    return text_height # Return vertical space used
...
def draw_button_status(surface, button_idx, button_state, x, y, available_width):
    text_height = draw_text(surface, f"Button {button_idx}:", x, y)

    # Visual Circle Configuration
    circle_radius = FONT_SIZE * 0.6
    circle_x = x + available_width - circle_radius - PADDING # Align circle to the right
...
def draw_hat_status(surface, hat_idx, hat_value, x, y):
    ...
    text_height = draw_text(surface, f"Hat {hat_idx}: {hat_value}", x, y)
...
while running:
    ...
    for i, joystick in enumerate(joysticks):
        ...
        name_height = draw_text(screen, controller_name, start_x, current_y, BLUE)
        ...
        if num_axes > 0:
            axis_title_h = draw_text(screen, "Axes:", start_x, current_y, WHITE)
            current_y += axis_title_h + 5
        ...
        if num_buttons > 0:
            button_title_h = draw_text(screen, "Buttons:", start_x, current_y, WHITE)
            current_y += button_title_h + 5
        ...
        if num_hats > 0:
            hat_title_h = draw_text(screen, "Hats (DPad):", start_x, current_y, WHITE)
            current_y += hat_title_h + 5
