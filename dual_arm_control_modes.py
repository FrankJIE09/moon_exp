...
import pygame
# 确保 CPS.py 中的所有必要内容都被导入，特别是 CPSClient, desire_left_pose, desire_right_pose
# 如果 desire_... 函数不在 CPS.py 中，你需要确保它们在此文件或其他导入的文件中定义
from CPS import *
import time
import numpy as np
...
# 注意：下面的 RESET_JOINT 定义现在不再直接用于回正移动，
# 但可能仍用于其他目的或作为参考。回正现在使用下面的笛卡尔逻辑。
RESET_JOINT_LEFT = [90.0, -45.0, 90.0, 0.0, 90.0, 0.0] # 示例左臂回正角度 (不再直接使用)
RESET_JOINT_RIGHT = [-90.0, -45.0, -90.0, 0.0, 90.0, 0.0] # 示例右臂回正角度 (不再直接使用)

# 定义回正时的目标姿态 RPY (工具垂直向下)
# !!! 请务必根据你的实际坐标系定义调整这些值 !!!
# 可能需要通过 desire_left_pose/desire_right_pose 或直接指定
...
def format_speed(speed_array):
    """格式化速度向量以便打印"""
    # ... (函数保持不变) ...
    if not hasattr(speed_array, '__len__') or len(speed_array) < 6: return "[无效速度数据]"
    try: return f"[{float(speed_array[0]):>6.1f}, {float(speed_array[1]):>6.1f}, {float(speed_array[2]):>6.1f}, {float(speed_array[3]):>6.1f}, {float(speed_array[4]):>6.1f}, {float(speed_array[5]):>6.1f}]"
    except (ValueError, IndexError): return "[格式化错误]"
...
def load_config(filepath):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f: config = yaml.safe_load(f)
        if config is None: raise ValueError("配置文件为空或格式错误")
        print(f"成功加载配置文件: {filepath}"); return config
    ...
    except Exception as e: print(f"加载配置文件时发生未知错误: {e}"); traceback.print_exc(); return None
...
def initialize_robot(controller, arm_name):
    ...
    try:
        ...
        print(f"--- {arm_name} 初始化完成 ---"); return True
...
def connect_arm_gripper(controller, arm_name):
    ...
    try:
        if controller.connect_gripper(): print(f"{arm_name} 夹爪连接并激活成功."); return True
        else: print(f"错误: {arm_name} 夹爪连接或激活失败."); return False
    except AttributeError: print(f"错误: 控制器对象无 'connect_gripper' 方法."); return False
    except Exception as e: print(f"连接 {arm_name} 夹爪时异常: {e}"); return False
...
class DualArmController:
    ...
    def __init__(self, config_path):
        """初始化控制器"""
        # ... (初始化大部分不变) ...
        self.config_path = config_path; self.config = None; self.joystick = None
        self.controller_left = None; self.controller_right = None; self.screen = None
        self.info_font = None; self.clock = None; self.sound_cache = {}; self.mixer_initialized = False
        self.num_hats = 0; self.num_axes = 0; self.num_buttons = 0
        self.running = False
        self.control_modes = [MODE_XYZ, MODE_RPY, MODE_RESET, MODE_VISION]
        self.current_mode_index = 0; self.control_mode = self.control_modes[self.current_mode_index]
        self.current_speed_xy = DEFAULT_XY_SPEED; self.current_speed_z = DEFAULT_Z_SPEED
        self.rpy_speed = DEFAULT_RPY_SPEED; self.reset_speed = DEFAULT_RESET_SPEED
        self.left_gripper_open = True; self.right_gripper_open = True
        self.left_gripper_active = False; self.right_gripper_active = False
        self.mode_button_press_time = None; self.left_init_ok = False; self.right_init_ok = False
        self.font_path = DEFAULT_FONT_PATH; self.left_robot_ip = DEFAULT_IP; self.right_robot_ip = DEFAULT_IP
        self.left_gripper_id = DEFAULT_GRIPPER_ID; self.right_gripper_id = DEFAULT_GRIPPER_ID
        self.window_width = DEFAULT_WINDOW_WIDTH; self.window_height = DEFAULT_WINDOW_HEIGHT
        self.font_size = DEFAULT_FONT_SIZE; self.speed_increment = DEFAULT_SPEED_INCREMENT
        self.min_speed = DEFAULT_MIN_SPEED; self.max_speed = DEFAULT_MAX_SPEED; self.acc = DEFAULT_ACC
        self.arot = DEFAULT_AROT; self.t_interval = DEFAULT_T; self.trigger_threshold = DEFAULT_TRIGGER_THRESHOLD
        self.gripper_speed = DEFAULT_GRIPPER_SPEED; self.gripper_force = DEFAULT_GRIPPER_FORCE
        self.long_press_duration = DEFAULT_LONG_PRESS_DURATION; self.controls_map = {}; self.audio_files_config = {}
        ...
        self.reset_left_arm_ctrl = None; self.reset_right_arm_ctrl = None
    ...
    def _load_config(self):
        ...
        try:
            ...
            setup_cfg = self.config.get('setup', {}); settings_cfg = self.config.get('settings', {})
            ...
            self.acc = settings_cfg.get('acc', self.acc); self.arot = settings_cfg.get('arot', self.arot)
            ...
            self.reset_speed = settings_cfg.get('reset_speed', DEFAULT_RESET_SPEED)
            self.controls_map = self.config.get('controls', {}); self.audio_files_config = self.config.get('audio_files', {})
            ...
            self.reset_left_arm_ctrl = self.controls_map.get('reset_left_arm')
            self.reset_right_arm_ctrl = self.controls_map.get('reset_right_arm')
            ...
            print("配置文件加载成功."); return True
        except FileNotFoundError: print(f"错误: 配置文件未找到: {self.config_path}"); return False
        except yaml.YAMLError as e: print(f"错误: 解析配置文件 {self.config_path} 失败: {e}"); return False
        except Exception as e: print(f"加载配置文件时发生未知错误: {e}"); traceback.print_exc(); return False
    ...
    def _init_pygame(self):
        ...
        try:
            ...
            try: pygame.mixer.init(); self.mixer_initialized = True; print("  Pygame Mixer 初始化成功."); self._load_sounds()
            except pygame.error as e: print(f"  错误：初始化 Mixer 失败: {e}"); self.mixer_initialized = False
            self.screen = pygame.display.set_mode((self.window_width, self.window_height))
            pygame.display.set_caption(f"双臂控制 (模式: {self.control_mode})")
            if self.font_path and os.path.exists(self.font_path):
                try: self.info_font = pygame.font.Font(self.font_path, self.font_size); print(f"  字体加载成功: {self.font_path}")
                except Exception as e: print(f"  字体加载失败: {e}"); self.info_font = None
            if self.info_font is None:
                print(f"  警告: 将使用默认字体.")
                try: self.info_font = pygame.font.Font(None, self.font_size)
                except Exception as e: raise RuntimeError(f"加载默认字体失败: {e}") from e
            ...
            self.num_hats = self.joystick.get_numhats(); self.num_axes = self.joystick.get_numaxes(); self.num_buttons = self.joystick.get_numbuttons()
            print(f"    能力: Hats={self.num_hats}, Axes={self.num_axes}, Buttons={self.num_buttons}")
            self.clock = pygame.time.Clock(); print("Pygame 初始化完成."); return True
        except Exception as e: print(f"Pygame 初始化失败: {e}"); traceback.print_exc(); return False
    ...
    def _load_sounds(self):
        ...
        print("\n正在加载音频文件...")
        loaded_count = 0
        sound_events_to_load = [
            'xyz_mode', 'rpy_mode', 'vision_enter', 'vision_exit',
            'left_open', 'left_close', 'right_open', 'right_close',
            'gripper_inactive', 'system_ready',
            'reset_mode',
            'left_reset_success', 'left_reset_fail',
            'right_reset_success', 'right_reset_fail'
        ]
        for event_name in sound_events_to_load:
            filepath = self.audio_files_config.get(event_name)
            if not filepath or not isinstance(filepath, str):
                 print(f"  警告: 配置文件中未找到 '{event_name}' 的音频路径或格式错误."); continue
            try:
                if os.path.exists(filepath): sound = pygame.mixer.Sound(filepath); self.sound_cache[event_name] = sound; loaded_count += 1
                else: print(f"  错误: 文件未找到 '{event_name}': {filepath}")
            except Exception as e: print(f"  错误: 加载 '{event_name}' ({filepath}) 时: {e}")
    ...
    def _play_sound(self, event_name):
        ...
        if sound:
            try: sound.play()
            except Exception as e: print(f"错误: 播放声音 '{event_name}' 时: {e}")
    ...
    def _init_robots(self):
        ...
        print("\n正在连接和初始化机器人...")
        try:
            print(f"连接左臂 ({self.left_robot_ip})..."); self.controller_left = CPSClient(self.left_robot_ip, gripper_slave_id=self.left_gripper_id)
            if not self.controller_left.connect(): raise ConnectionError("左臂连接失败")
            print("左臂连接成功."); self.left_init_ok = initialize_robot(self.controller_left, "左臂")
            if self.left_init_ok: self.left_gripper_active = connect_arm_gripper(self.controller_left, "左臂")
            print(f"连接右臂 ({self.right_robot_ip})..."); self.controller_right = CPSClient(self.right_robot_ip, gripper_slave_id=self.right_gripper_id)
            if not self.controller_right.connect(): raise ConnectionError("右臂连接失败")
            print("右臂连接成功."); self.right_init_ok = initialize_robot(self.controller_right, "右臂")
            ...
            print("机器人初始化流程结束。")
        except Exception as e: print(f"机器人连接或初始化过程中断: {e}"); traceback.print_exc(); self.left_init_ok = False; self.right_init_ok = False
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                if self.mode_switch_control.get('type') == 'button' and button_index == self.mode_switch_control.get('index', -1):
                    if self.mode_button_press_time is None: self.mode_button_press_time = current_time
                    ...
                    if self.left_gripper_active: sound_event = 'left_close' if self.left_gripper_open else 'left_open'; self._play_sound(sound_event); (self.controller_left.close_gripper if self.left_gripper_open else self.controller_left.open_gripper)(speed=self.gripper_speed, force=self.gripper_force, wait=False); self.left_gripper_open = not self.left_gripper_open
                    ...
                    if self.right_gripper_active: sound_event = 'right_close' if self.right_gripper_open else 'right_open'; self._play_sound(sound_event); (self.controller_right.close_gripper if self.right_gripper_open else self.controller_right.open_gripper)(speed=self.gripper_speed, force=self.gripper_force, wait=False); self.right_gripper_open = not self.right_gripper_open
                    else: self._play_sound('gripper_inactive')
                if self.control_mode == MODE_RESET:
                    ...
                    if self.reset_left_arm_ctrl and self.reset_left_arm_ctrl.get('type') == 'button' and button_index == left_reset_idx: print("左臂回正按钮按下..."); self._attempt_reset_left()
                    elif self.reset_right_arm_ctrl and self.reset_right_arm_ctrl.get('type') == 'button' and button_index == right_reset_idx: print("右臂回正按钮按下..."); self._attempt_reset_right()
    ...
    # --- MODIFICATION START: Updated Reset Methods ---
    def _attempt_reset_left(self):
        ...
        print(f"尝试将左臂回正到垂直姿态...")
        try:
            current_pose = self.controller_left.getTCPPose()
            if current_pose is None:
                print("错误：无法获取左臂当前 TCP 位姿。")
                self._play_sound('left_reset_fail')
                return

            # --- 使用你提供的逻辑 ---
            # 注意: desire_left_pose 需要能从 CPS.py 中导入或在此处定义
            # TARGET_RESET_RPY_LEFT 在文件顶部定义
            try:
                # 如果 desire_left_pose 是必要的转换函数
                # rpy_angles = desire_left_pose(rpy_array=TARGET_RESET_RPY_LEFT)
                # 如果 TARGET_RESET_RPY_LEFT 就是最终想要的欧拉角
                rpy_angles = desire_left_pose(rpy_array=TARGET_RESET_RPY_LEFT)  # left 垂直向下
            except NameError:
                 print("错误: 函数 desire_left_pose 未定义!")
                 self._play_sound('left_reset_fail')
                 return
            except Exception as e_desire:
                 print(f"错误: 调用 desire_left_pose 时出错: {e_desire}")
                 self._play_sound('left_reset_fail')
                 return

            target_pose = list(current_pose) # 创建副本
            target_pose[3:6] = rpy_angles    # 仅修改姿态部分
            print(f"  目标位姿 (仅姿态): {target_pose}")

            # 使用 move_robot (内部处理 IK 和 moveByJoint)
            success = self.controller_left.move_robot(target_pose, speed=self.reset_speed, block=True)
            # --- END 使用你提供的逻辑 ---

            if success:
                print("左臂回正成功。")
                self._play_sound('left_reset_success')
            else:
                print("左臂回正失败。 (move_robot 返回 False)")
                self._play_sound('left_reset_fail')

        except AttributeError as ae:
             print(f"左臂回正错误: 控制器对象缺少方法 (可能是 getTCPPose 或 move_robot): {ae}")
             traceback.print_exc()
             self._play_sound('left_reset_fail')
    ...
    def _attempt_reset_right(self):
        ...
        print(f"尝试将右臂回正到垂直姿态...")
        try:
            current_pose = self.controller_right.getTCPPose()
            if current_pose is None:
                print("错误：无法获取右臂当前 TCP 位姿。")
                self._play_sound('right_reset_fail')
                return

            # --- 使用你提供的逻辑 ---
            # 注意: desire_right_pose 需要能从 CPS.py 中导入或在此处定义
            # TARGET_RESET_RPY_RIGHT 在文件顶部定义
            try:
                # 如果 desire_right_pose 是必要的转换函数
                # rpy_angles = desire_right_pose(rpy_array=TARGET_RESET_RPY_RIGHT)
                # 如果 TARGET_RESET_RPY_RIGHT 就是最终想要的欧拉角
                rpy_angles = desire_right_pose(rpy_array=TARGET_RESET_RPY_RIGHT)  # left 垂直向下
            except NameError:
                 print("错误: 函数 desire_right_pose 未定义!")
                 self._play_sound('right_reset_fail')
                 return
            except Exception as e_desire:
                 print(f"错误: 调用 desire_right_pose 时出错: {e_desire}")
                 self._play_sound('right_reset_fail')
                 return
            ...
            # 使用 move_right_robot (内部处理 IK 和 moveByJoint_right)
            success = self.controller_right.move_right_robot(target_pose, speed=self.reset_speed, block=True)
             # --- END 使用你提供的逻辑 ---

            if success:
                print("右臂回正成功。")
                self._play_sound('right_reset_success')
            else:
                print("右臂回正失败。 (move_right_robot 返回 False)")
                self._play_sound('right_reset_fail')

        except AttributeError as ae:
             print(f"右臂回正错误: 控制器对象缺少方法 (可能是 getTCPPose 或 move_right_robot): {ae}")
             traceback.print_exc()
             self._play_sound('right_reset_fail')
        except Exception as e:
    ...
    def _get_joystick_input_state(self, control_config):
        ...
        active = False; ctrl_type = control_config.get('type'); ctrl_index = control_config.get('index', -1)
        ...
        try:
            if ctrl_type == 'button': active = self.joystick.get_button(ctrl_index) == 1
            elif ctrl_type == 'axis':
                axis_val = self.joystick.get_axis(ctrl_index); threshold = control_config.get('threshold', self.trigger_threshold); direction = control_config.get('direction', 1)
                if direction == 1 and axis_val > threshold: active = True
                elif direction == -1 and axis_val < -threshold: active = True
            elif ctrl_type == 'hat':
                hat_val = self.joystick.get_hat(ctrl_index); hat_axis = control_config.get('axis', 'x'); direction = control_config.get('direction', 1)
                if hat_axis == 'x' and hat_val[0] == direction: active = True
                elif hat_axis == 'y' and hat_val[1] == direction: active = True
        except pygame.error as e: print(f"读取手柄输入时出错({ctrl_type},{ctrl_index}): {e}"); return False
        return active

    def _calculate_speed_commands(self):
        """根据当前模式和输入计算原始速度指令"""
        # ... (方法保持不变) ...
        speed_left_cmd = np.zeros(6); speed_right_cmd = np.zeros(6); current_mode_prefix = self.control_mode.lower() + "_"
                ...
                if action.startswith(current_mode_prefix) and ('_arm' in action):
                     if self._get_joystick_input_state(control):
                        ...
                        if target_speed_array is not None:
                            ...
                            if axis_index != -1:
                                if control.get('type') == 'axis': axis_val = self.joystick.get_axis(control.get('index', -1)); scaled_speed = base_speed * abs(axis_val); target_speed_array[axis_index] = scaled_speed * direction_sign
                                else: target_speed_array[axis_index] = base_speed * direction_sign
        elif self.control_mode == MODE_VISION: run_vision_mode(); speed_left_cmd = np.zeros(6); speed_right_cmd = np.zeros(6)
        elif self.control_mode == MODE_RESET: speed_left_cmd = np.zeros(6); speed_right_cmd = np.zeros(6)
        return speed_left_cmd, speed_right_cmd

    def _apply_transformations(self, speed_left_cmd, speed_right_cmd):
        """对速度指令应用坐标变换"""
        # ... (方法保持不变) ...
        speed_left_final = np.zeros(6); speed_right_final = np.zeros(6)
        if self.control_mode == MODE_XYZ:
            ...
            try: rot_right = R.from_euler('xyz', [65.334, -4.208, -9.079], degrees=True).as_matrix(); speed_right_final[0:3] = speed_right_cmd[0:3] @ rot_right
            ...
            speed_left_final[0:3] = [0.0, 0.0, 0.0]; speed_right_final[0:3] = [0.0, 0.0, 0.0]
            speed_left_final[3:] = speed_left_cmd[3:]; speed_right_final[3:] = speed_right_cmd[3:]
        elif self.control_mode in [MODE_RESET, MODE_VISION]: speed_left_final = np.zeros(6); speed_right_final = np.zeros(6)
    ...
    def _draw_display(self, speed_left_final, speed_right_final):
        ...
        self.screen.fill(C_BLACK); lines_to_draw = []; y_pos = INFO_Y_START
        fmt_speed_l = format_speed(speed_left_final); fmt_speed_r = format_speed(speed_right_final)
        ...
        if self.control_mode in [MODE_XYZ, MODE_RPY]:
             ...
             lines_to_draw.append(("-")); lines_to_draw.append(("[当前速度指令 (已发送)]"))
             lines_to_draw.append((f"  左臂速度: {fmt_speed_l}")); lines_to_draw.append((f"  右臂速度: {fmt_speed_r}")); lines_to_draw.append(("-"))
        elif self.control_mode == MODE_RESET:
             lines_to_draw.append((f"回正速度: {self.reset_speed:.1f}", C_BLUE)); lines_to_draw.append(("-")); lines_to_draw.append(("[回正模式]"))
             left_reset_btn_idx = self.reset_left_arm_ctrl.get('index', '?') if self.reset_left_arm_ctrl else '?'; right_reset_btn_idx = self.reset_right_arm_ctrl.get('index', '?') if self.reset_right_arm_ctrl else '?'
             lines_to_draw.append((f"  按下按钮 B{left_reset_btn_idx} 左臂回正", C_CYAN)); lines_to_draw.append((f"  按下按钮 B{right_reset_btn_idx} 右臂回正", C_CYAN)); lines_to_draw.append(("-"))
        elif self.control_mode == MODE_VISION: lines_to_draw.append(("-")); lines_to_draw.append(("[视觉模式活动]")); lines_to_draw.append(("  等待视觉处理...", C_GRAY)); lines_to_draw.append(("-"))
        mode_switch_btn_idx = self.mode_switch_control.get('index', '?'); gripper_l_btn_idx = self.gripper_toggle_left_ctrl.get('index', '?') if self.gripper_toggle_left_ctrl else '?'; gripper_r_btn_idx = self.gripper_toggle_right_ctrl.get('index', '?') if self.gripper_toggle_right_ctrl else '?'
        hints = f"提示: B{mode_switch_btn_idx}切换 | 夹爪L(B{gripper_l_btn_idx}) R(B{gripper_r_btn_idx})"
        if self.control_mode in [MODE_XYZ, MODE_RPY]: speed_dec_btn_idx = self.speed_dec_control.get('index', '?'); speed_inc_btn_idx = self.speed_inc_control.get('index', '?'); hints += f" | 速度(B{speed_dec_btn_idx}/B{speed_inc_btn_idx})"
        ...
        for item in lines_to_draw:
            ...
            try:
                ...
                text_surface = self.info_font.render(line_text, True, line_color); self.screen.blit(text_surface, (INFO_X_MARGIN, y_pos))
    ...
    def run(self):
        ...
        self._init_robots()
        self._play_sound('system_ready'); self.running = True
        print("\n--- 控制循环开始 ---")
        while self.running:
            self._handle_events()
            ...
            if self.left_init_ok or self.right_init_ok: self._send_robot_commands(speed_left_final, speed_right_final)
            self._draw_display(speed_left_final, speed_right_final); self.clock.tick(30)
    ...
    def cleanup(self):
        ...
        try:
            if self.left_init_ok and self.controller_left and self.controller_left.sock:
                ...
                time.sleep(0.1); self.controller_left.disconnect(); print("  左臂已断开。")
            if self.right_init_ok and self.controller_right and self.controller_right.sock:
                ...
                time.sleep(0.1); self.controller_right.disconnect(); print("  右臂已断开。")
        except Exception as stop_e: print(f"  断开连接时出错: {stop_e}")
        if self.mixer_initialized: pygame.mixer.quit(); print("Pygame Mixer 已退出。")
        pygame.quit(); print("程序退出。")

# -- 主程序入口 --
if __name__ == "__main__":
    print("启动双臂控制器...")
    controller_app = DualArmController(CONFIG_FILE)
    try: controller_app.run()
    except KeyboardInterrupt: print("\n检测到手动中断 (Ctrl+C)...")
    except Exception as e: print("\n主程序运行时发生未捕获的异常:"); traceback.print_exc()
    finally: controller_app.cleanup()